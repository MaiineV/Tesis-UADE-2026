...
namespace Rollgeon.Grid.Tests
{
    public class RenderTextureCursorTests
    {
        ...
        [Test]
        public void test_rtToScreen_upscalesRtPointToScreenPixels()
        {
            // Arrange — RT a mitad de resolución: un punto del RT mapea al doble en pantalla.
            var rt = new Vector2(480f, 270f);

            // Act
            var result = RenderTextureCursor.RtToScreen(rt, 1920f, 1080f, 960f, 540f);
        ...
        [Test]
        public void test_rtToScreen_roundTripsWithScreenToRt()
        {
            // Arrange
            var screen = new Vector2(1234f, 567f);

            // Act — screen → RT → screen debe devolver el punto original.
            var rt = RenderTextureCursor.ScreenToRt(screen, 1920f, 1080f, 320f, 180f);
            var back = RenderTextureCursor.RtToScreen(rt, 1920f, 1080f, 320f, 180f);

            // Assert
            Assert.AreEqual(screen.x, back.x, 1e-3f);
            Assert.AreEqual(screen.y, back.y, 1e-3f);
        }

        [Test]
        public void test_rtToScreen_zeroRtDim_returnsZeroWithoutDivideByZero()
        {
            // Act — RT degenerado no debe producir NaN/Inf.
            var result = RenderTextureCursor.RtToScreen(new Vector2(100f, 100f), 1920f, 1080f, 0f, 0f);
