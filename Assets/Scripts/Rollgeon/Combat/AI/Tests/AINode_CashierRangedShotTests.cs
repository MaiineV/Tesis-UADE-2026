...
{
    // La tenaza: pegarle al jefe exige distancia 1, así que el rango del disparo no puede bajar de
    // la distancia a la que kitea (AINode_KeepDistance.IdealDistance = 4) o se repliega fuera de su
    // propio alcance y vuelve a ser el que no podía pegar.
    [TestFixture]
    public class AINode_CashierRangedShotTests
    {
        ...
        {
            // El jugador está exactamente a 4, la distancia a la que el jefe kitea.
            var node = NewNode();
        ...
        {
            // La única distancia desde la que el jugador puede pegarle.
        ...
        {
            // Diagonal (3,3): Chebyshev 3 (dentro) pero Manhattan 6 (fuera).
        ...
        {
            // Un asset mal autorado no debe generar golpes de 0 en el log de combate.
            var node = NewNode();
            node.Damage = 0;
