...
- **§13 Cruce de sala con paneo (Feature#0086)** — en modo estático el cruce de puerta ya no es un corte seco. `DungeonManager.TransitionTo` deja **activa la sala saliente** (`IDungeonService.LeavingRoomInstance`) y, tras `OnRoomEntered`, emite `OnRoomCrossed(from, to)`. El `CameraService` consume ese evento en el reanclado diferido (`ApplyPendingReanchor`, `LateUpdate`) y en vez de snapear **panea el foco** del centro de la sala saliente al de la nueva (`CameraConfigSO.RoomPanSeconds` = 0.35 s, `RoomPanEase`; snap con reduced motion o `RoomPanSeconds = 0`). Al aterrizar emite `OnCameraRoomPanFinished(roomId)` y recién ahí el `DungeonManager` apaga la saliente. Sin `ICameraService` registrado (tests, headless) la saliente se apaga en el acto, como antes. La primera sala del piso y el resume de save no emiten `OnRoomCrossed` → snap. El toast de sala (`RoomEnterToastView`, `Canvas_Toast`) también escucha `OnRoomCrossed`: sube desde el borde inferior con tipo + nombre de sala, sostiene `_holdSeconds / GameSpeedPrefs.Multiplier` y baja solo. **Por ahora queda apagado** (el componente está cableado pero deshabilitado en el prefab); se prende tildando el componente en `Canvas_Toast/ToastCanvas`. El paneo **no** se divide por la velocidad de juego (animación, no pacing). El jugador sigue snapeando a la sala nueva en el mismo frame (`PlayerRoomTransitioner`).
